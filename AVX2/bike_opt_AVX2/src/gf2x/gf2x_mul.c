...
void gf2x_mul_4096(OUT uint64_t *c, IN const uint64_t *a, IN const uint64_t *b)
{
  ...
  karatzuba_ches((uint64_t *)c, (const uint64_t *)a, (const uint64_t *)b, 64, 64, secure_buffer);
...
void gf2x_mul_8192(OUT uint64_t *c, IN const uint64_t *a, IN const uint64_t *b)
{
  ...
  karatzuba_ches((uint64_t *)c, (const uint64_t *)a, (const uint64_t *)b, 128, 128, secure_buffer);
...
#if VPCLMUL == 0
#if LEVEL == 1
  gfmul_49_pclmul((__m256i *)&t,  (const __m256i *)a,   (const __m256i *)b);
#elif LEVEL == 3
  gfmul_97_pclmul((__m256i *)&t,  (const __m256i *)a,   (const __m256i *)b);
#elif LEVEL == 5
  gfmul_161_pclmul((__m256i *)&t,  (const __m256i *)a,   (const __m256i *)b);
#endif
#elif VPCLMUL == 1
#if LEVEL == 1
  gfmul_49_vpclmul((__m256i *)&t,  (const __m256i *)a,   (const __m256i *)b);
#elif LEVEL == 3
  gfmul_97_vpclmul((__m256i *)&t,  (const __m256i *)a,   (const __m256i *)b);
#elif LEVEL == 5
  gfmul_161_vpclmul((__m256i *)&t,  (const __m256i *)a,   (const __m256i *)b);
#endif
#endif

#elif GFMUL_VER == 2 //CHES
  DEFER_CLEANUP(dbl_pad_r_t t = {0}, dbl_pad_r_cleanup);
#if R_BITS == 12323
  //printf("CHES");
  rkara3_mul_12352((uint64_t *)&t, (const uint64_t *)a, (const uint64_t *)b);
#elif  R_BITS == 24659
  rkara3_mul_24704((uint64_t *)&t, (const uint64_t *)a, (const uint64_t *)b);
#else
//error here.
  // karatzuba((uint64_t *)&t, (const uint64_t *)a, (const uint64_t *)b, R_QWORDS,
...
#elif GFMUL_VER == 3 //gf2xlib
  DEFER_CLEANUP(dbl_pad_r_t t = {0}, dbl_pad_r_cleanup);
#if VPCLMUL == 0
#if R_BITS == 12323
  gf2x_mul((uint64_t *)&t, (const uint64_t *)a, 193, (const uint64_t *)b,193);
#elif  R_BITS == 24659
  gf2x_mul((uint64_t *)&t, (const uint64_t *)a, 386, (const uint64_t *)b,386);

#else
  gf2x_mul((uint64_t *)&t, (const uint64_t *)a, 641, (const uint64_t *)b,641);
#endif
#endif
#endif
  ctx->red(c, &t);
