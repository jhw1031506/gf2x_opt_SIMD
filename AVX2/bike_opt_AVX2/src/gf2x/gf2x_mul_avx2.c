...
_INLINE_ void karatzuba_add1_ches(OUT uint64_t *alah,
                             OUT uint64_t *blbh,
                             IN const uint64_t *a,
                             IN const uint64_t *b,
...
_INLINE_ void karatzuba_add2_ches(OUT uint64_t *z,
                             IN const uint64_t *x,
                             IN const uint64_t *y,
                             IN const size_t    qwords_len)
{
...
_INLINE_ void karatzuba_add3_ches(OUT uint64_t *c,
...
// completion.
void karatzuba_ches(OUT uint64_t *c,
                        IN const uint64_t *a,
                        IN const uint64_t *b,
                        IN const size_t    qwords_len,
                        ...
                        uint64_t *         sec_buf)
{
  ...
  karatzuba_ches(c0, a_lo, b_lo, half_qw_len, half_qw_len, sec_buf);
  ...
  if(qwords_len > half_qw_len) {
    ...
    karatzuba_ches(c2, a_hi, b_hi, qwords_len - half_qw_len, half_qw_len, sec_buf);

    // Compute alah = (a_lo + a_hi) and blbh = (b_lo + b_hi)
    karatzuba_add1_ches(alah, blbh, a, b, half_qw_len);

    // Compute (c1 + c2) and store the result in tmp
    karatzuba_add2_ches(tmp, c1, c2, half_qw_len);

    // Compute alah*blbh and store the result in (c2|c1)
    karatzuba_ches(c1, alah, blbh, half_qw_len, half_qw_len, sec_buf);

    // Add (tmp|tmp) and (c3|c0) to (c2|c1)
    karatzuba_add3_ches(c0, tmp, half_qw_len);
  }
}
