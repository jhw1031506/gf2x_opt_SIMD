...
#define LOAD256(mem)       _mm256_loadu_si256((const void *)(mem))
#define STORE128(mem, reg) _mm_storeu_si128((void *)(mem), (reg))
#define STORE256(mem, reg) _mm256_storeu_si256((void *)(mem), (reg))
                            ...
                            IN const __m128i b_hi)
{
	...
	//	Compute Al.Bl=D0
	__m128i DD0 = _mm_clmulepi64_si128(a_lo, b_lo, 0);//a0*b0
	__m128i DD2 = _mm_clmulepi64_si128(a_lo, b_lo, 0x11);//a1*b1
	__m128i AAlpAAh = _mm_xor_si128(a_lo, _mm_shuffle_epi32(a_lo, 0x4e));//shuffle 0x4e : a1a0->a0a1 (a3,a4:64bit)
	__m128i BBlpBBh = _mm_xor_si128(b_lo, _mm_shuffle_epi32(b_lo, 0x4e));
	...
	//	Compute Ah.Bh=D2
	DD0 = _mm_clmulepi64_si128(a_hi, b_hi, 0);//a2*b2
	DD2 = _mm_clmulepi64_si128(a_hi, b_hi, 0x11);//a3*b3 
	AAlpAAh = _mm_xor_si128(a_hi, _mm_shuffle_epi32(a_hi, 0x4e));//shuffle 0x4e : a3a2->a2a3 (a2,a3:64bit)
	BBlpBBh = _mm_xor_si128(b_hi, _mm_shuffle_epi32(b_hi, 0x4e));//xor_si128 : xor
	...
	__m128i AlpAh = _mm_xor_si128(a_lo,a_hi);
	__m128i BlpBh = _mm_xor_si128(b_lo,b_hi);
...
#endif
#elif VPCLMUL==1
_INLINE_ void gf2x_mul4_int(OUT __m256i      c[2],
                            IN const __m256i a,
                            IN const __m256i b)
{
                          ...
                          IN const uint64_t *b)
{
  __m256i va[2], vb[2];
  __m256i aa, bb;
  __m256i lo[2], hi[2], mi[2], m;

  for(size_t i = 0; i < 2; i++) {
    va[i] = LOAD256(&a[QWORDS_IN_YMM * i]);
    vb[i] = LOAD256(&b[QWORDS_IN_YMM * i]);
  }

  // Multiply the low and the high halves of a and b
  // lo <-- a_lo * b_lo
  // hi <-- a_hi * b_hi
  gf2x_mul4_int(lo, va[0], vb[0]);
  gf2x_mul4_int(hi, va[1], vb[1]);
  ...
  aa = va[0] ^ va[1];
  bb = vb[0] ^ vb[1];
  gf2x_mul4_int(mi, aa, bb);

  m = lo[1] ^ hi[0];

  STORE256(&c[0 * QWORDS_IN_YMM], lo[0]);
  STORE256(&c[1 * QWORDS_IN_YMM], mi[0] ^ lo[0] ^ m);
  STORE256(&c[2 * QWORDS_IN_YMM], mi[1] ^ hi[1] ^ m);
  STORE256(&c[3 * QWORDS_IN_YMM], hi[1]);
}
#endif
