...
void stack_push_inner_dim(uint32_t * stk, uint32_t * stk_len, uint32_t p_index, uint8_t * flag_TC){
    ...
    else if(best_alg[parents_dim] == 4){ //TC3_128
        flag_TC[0] = 1;
        uint32_t k = parents_dim/3;
        if(parents_dim % 3 == 0){
            child_cnt = 1;
    ...
    else if(best_alg[parents_dim] == 5){ //TC3_64
        flag_TC[1] = 1;
        uint32_t k = parents_dim/3;
        if(parents_dim % 3 == 0){
            child_cnt = 2;
...
void gfopt_codegen(uint32_t len){
    ...
    //flag_TC[2]: TC64
    uint8_t flag_TC[3]={0};
    printf("generating gfopt multiplication code for len = %d..\n", len);
    if(len > MAX_LEN){
...
//basemul (len = 1)
    ...
    switch(best_alg[1]){
    case 0:
        fprintf(fp, "//len = 0: karat_NEON\n");
        fprint_karat_128_NEON(fp,func_name);
        break;
    case 1:
        fprintf(fp, "//len = 1: SB_NEON\n");
        fprint_SB_128_NEON(fp,func_name);
        break;
    default:
        printf("err. best_alg[1] = %d\n", best_alg[1]);
    }
...
int main(int argc, char *argv[]){
    ...
    #ifdef ENV_AVX2
    strcat(str_options, " -mpclmul -msse2 -mavx2 ");
    #ifdef ENV_VPCLMUL
