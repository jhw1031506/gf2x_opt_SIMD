...
#define SECURE_BUFFER_128 (3 * R_PADDED_128)
...
#if GFMUL_VER == 0 || GFMUL_VER == 1 // original (port, NEON)
...
// #elif GFMUL_VER == 1 // original Neon
  
//   DEFER_CLEANUP(dbl_pad_r_t t = {0}, dbl_pad_r_cleanup);
//   ALIGN(ALIGN_BYTES) uint64_t secure_buffer[SECURE_BUFFER_QWORDS];
//   karatzuba_NEON((uint64_t *)&t, (const uint64_t *)a, (const uint64_t *)b, R_QWORDS,
//             R_PADDED_QWORDS, secure_buffer, ctx);
//   secure_clean((uint8_t *)secure_buffer, sizeof(secure_buffer));
  ...
  DEFER_CLEANUP(dbl_pad_r_t t = {0}, dbl_pad_r_cleanup);
#if LEVEL == 1
  gfmul_97((poly8x16_t *)&t,  (const poly8x16_t *)a,   (const poly8x16_t  *)b);
#elif LEVEL == 3
  gfmul_193((poly8x16_t *)&t,  (const poly8x16_t  *)a,   (const poly8x16_t  *)b);
#elif LEVEL == 5
  gfmul_321((poly8x16_t *)&t,  (const poly8x16_t  *)a,   (const poly8x16_t  *)b);
#endif
