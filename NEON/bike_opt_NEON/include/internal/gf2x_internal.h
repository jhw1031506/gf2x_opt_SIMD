...
#define GF2X_NEON_BASE_QWORDS    (2)
...
#if GFMUL_VER == 1 || GFMUL_VER == 2 
void gf2x_mul_base_NEON(OUT uint64_t *c,
                        IN const uint64_t *a,
                        IN const uint64_t *b);
...
// The k-squaring function computes c = a^(2^k) % (x^r - 1),
// It is required by inversion, where l_param is derived from k.
void k_sqr_NEON(OUT pad_r_t *c, IN const pad_r_t *a, IN size_t l_param);
// c = a mod (x^r - 1)
...
#if GFMUL_VER == 1 || GFMUL_VER == 2
  {
    ctx->karatzuba_add1 = karatzuba_add1_NEON;
    ctx->karatzuba_add2 = karatzuba_add2_NEON;
    ctx->karatzuba_add3 = karatzuba_add3_NEON;
    ctx->k_sqr          = k_sqr_port;
    ctx->red            = gf2x_red_NEON;
  }
  {
    ctx->mul_base_qwords = GF2X_NEON_BASE_QWORDS;
    ctx->mul_base        = gf2x_mul_base_NEON;
    ctx->sqr             = gf2x_sqr_NEON;
  }
